...
-- race here ----------------------------------------------------------------------------------------------------------------------------------------        
        elements[#elements + 1] = {
            label = T.MenuCreation.element3.label,
            value = "race",
            desc = imgPath:format(img3) .. "<br> " .. T.MenuCreation.element3.desc,
        }
-------------------------------------------------------------------------------------------------------------------------------------------------------        
        elements[#elements + 1] = {
            label = __CHARNAME or T.MenuCreation.element4.label,
            value = __VALUE1 or "name",
            desc = __DESC or imgPath:format(img4) .. "<br> " .. T.MenuCreation.element4.desc,
        }
        elements[#elements + 1] = {
            label = __LABEL or ("<span style='color: Grey;'>" .. T.MenuCreation.element5.label .. "</span>"),
            value = __VALUE or "not",
            desc = imgPath:format(img3) .. "<br> " .. T.MenuCreation.element5.desc,
        }
    else
        -- enable clothing
        elements[#elements + 1] = {
...
-- ------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if (data.current.value == "save") then
                menu.close()
                --* name character
                TriggerServerEvent("vorpcharacter:saveCharacter", PlayerSkin, PlayerClothing, FirstName, LastName, MyRace)
                --TriggerServerEvent("vorpcharacter:saveCharacter", PlayerSkin, PlayerClothing, FirstName, LastName) added race^^^ this is old----------------------------------------------------
                CachedComponents = PlayerClothing
                CachedSkin = PlayerSkin
                __CloseAll()
            end
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        {
            ...
            subtext = "Select your character's race",
            align = Config.Align,
            elements = elements,
        },

        function(data, menu)
            local selectedRace = data.current.value
            print('race =', selectedRace)
            HandleRaceSelection(selectedRace)
            menu.close()
        end function(data, menu)
